...
var EXPORTED_SYMBOLS = [
  ...
  const {classes: Ccl, interfaces: Cis, utils: Cus, results : Crs} = Components;
...
Cus.import("resource://gre/modules/XPCOMUtils.jsm"); // for defineLazyServiceGetter
Cus.import("resource:///modules/gloda/mimemsg.js");
Cus.import("resource:///modules/gloda/utils.js");
Cus.import("resource:///modules/iteratorUtils.jsm"); // for toXPCOMArray
Cus.import("resource:///modules/mailServices.js");
...
XPCOMUtils.defineLazyGetter(MailServices, "messenger", function () {
  return Ccl["@mozilla.org/messenger;1"].createInstance(Cis.nsIMessenger);
});
...
function msgHdrFromNeckoUrl(aUrl)
  aUrl.QueryInterface(Cis.nsIMsgMessageUrl).messageHeader
...
function msgHdrToMessageBody(aMessageHeader, aStripHtml, aLength) {
  let messenger = Ccl["@mozilla.org/messenger;1"].createInstance(Cis.nsIMessenger);
  let listener = Ccl["@mozilla.org/network/sync-stream-listener;1"].createInstance(Cis.nsISyncStreamListener);
...
function msgHdrSetTags (aMsgHdr, aTags) {
  ...
  let msgHdr = toXPCOMArray([aMsgHdr], Cis.nsIMutableArray);
  ...
  for each (let msgHdr in msgHdrs) {
    ...
    if (!pending[msgHdr.folder.URI]) {
      pending[msgHdr.folder.URI] = {
        ...
        msgs: Ccl["@mozilla.org/array;1"].createInstance(Cis.nsIMutableArray)
      };
    }
...
function getMail3Pane() {
  return Ccl["@mozilla.org/appshell/window-mediator;1"]
          .getService(Cis.nsIWindowMediator)
...
function msgHdrIsRss(msgHdr)
  (msgHdr.folder.server instanceof Cis.nsIRssIncomingServer)
...
function msgHdrIsNntp(msgHdr)
  (msgHdr.folder.server instanceof Cis.nsINntpIncomingServer)
...
function msgHdrIsJunk(aMsgHdr)
  aMsgHdr.getStringProperty("junkscore") == Cis.nsIJunkMailPlugin.IS_SPAM_SCORE
...
function createStreamListener(k) {
  return {
    ...
    QueryInterface:
      XPCOMUtils.generateQI([Cis.nsIStreamListener, Cis.nsIRequestObserver]),
    ...
    onDataAvailable: function(aRequest, aContext, aInputStream, aOffset, aCount) {
      if (this._stream == null) {
        this._stream = Ccl["@mozilla.org/scriptableinputstream;1"].createInstance(Cis.nsIScriptableInputStream);
